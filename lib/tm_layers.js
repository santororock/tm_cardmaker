...
function getUniqueAutomaticLayerName(title) {
    const baseName = String(title || "Layer").replace(/\s+/g, " ").trim() || "Layer";
    const existingNames = new Set();
    if (typeof aLayers !== "undefined" && aLayers) {
        Object.keys(aLayers).forEach(function(id) {
            const existing = aLayers[id];
            if (existing && existing.name) existingNames.add(existing.name.toLowerCase());
        });
    }
...
function addLayer(title, layer, options) {
    ...
    if (!layer.name) {
        layer.name = getUniqueAutomaticLayerName(title);
    }
...
/* Personal text shortcuts are browser preferences, independent of project files. */
(function () {
    'use strict';
    const storageKey = 'tm_cardmaker_text_symbols';
    const defaults = '€\n°';

    document.addEventListener('DOMContentLoaded', function () {
        const library = document.getElementById('textSymbolLibrary');
        // Startup detaches this movable panel from the document until a layer
        // is selected. Query the panel itself so initialization also works then.
        const bar = domParams.querySelector('#textSymbolBar');
        const editor = domParams.querySelector('#inputdata');
        const status = document.getElementById('textSymbolStatus');
        let saved = defaults;
        try {
            saved = localStorage.getItem(storageKey) ?? defaults;
        } catch (error) {
            status.textContent = 'Browser storage is unavailable. Symbols will only last for this session.';
        }
        library.value = saved;

        function render() {
            bar.replaceChildren();
            const symbols = [...new Set(library.value.split(/\r?\n/).map(value => value.trim()).filter(Boolean))];
            for (const symbol of symbols) {
                ...
                // Preserve the text selection when clicking with a mouse.
                ...
                button.addEventListener('click', function () {
                    ...
                    // Use the normal layer update, rendering, autosave and undo path.
            ...
            customize.textContent = 'Edit symbols…';
        ...
        function save() {
            render();
            try {
                localStorage.setItem(storageKey, library.value);
                status.textContent = 'Saved in this browser.';
            } catch (error) {
                status.textContent = 'Could not save in this browser. Changes will only last for this session.';
            }
        }
        library.addEventListener('input', save);
        document.getElementById('resetTextSymbols').addEventListener('click', function () {
            library.value = defaults;
            save();
        });
        render();
    });
