...
(function() {
    ...
    */
        ...
        */
        ...
        drawOverlay(mainCanvas, layersArray, allLayersObj) {
            ...
            if (this.mode === 'selected') {
                const selectedIds = typeof window.getSelectedLayerIds === 'function'
                    ? window.getSelectedLayerIds(true)
                    : Array.from(document.querySelectorAll('.divRec.selected')).map(function(row) { return row.id; });
                targets = selectedIds.map(function(id) { return allLayersObj[id]; }).filter(function(layer) {
                    return layer && layer.type !== 'group';
                });
            } else {
                targets = Array.isArray(layersArray) ? layersArray.slice() : [];
            }

            for (let layer of targets) {
                let layerId = null;
                for (let id in allLayersObj)
                    if (allLayersObj[id] === layer) { layerId = id; break; }
                if (!layerId) continue;
                ...
                if (layer.type === 'text' && window.calculateTextBounds) {
                    ...
                    bounds = getRotatedRectBounds(layer);
                }

                // Draw bounding box (AABB for rotated layers, regular rect otherwise)
                this.octx.strokeStyle = 'red';
                this.octx.strokeRect(bounds.x, bounds.y, bounds.width, bounds.height);

                // If rotated, also draw the actual rotated rectangle
                ...
                try {
                    ...
                    if (layer.type === 'text') {
                        ...
                        if (b && b.obj && b.obj.complete) {
                            ...
                            if (mask) {
                                drawMaskOnOverlay(this.octx, mask, layer);
                                drewMask = true;
                            }
                        }
                ...
                this.octx.beginPath();
                this.octx.moveTo(bounds.x - 6, bounds.y);
                this.octx.lineTo(bounds.x + 6, bounds.y);
                this.octx.moveTo(bounds.x, bounds.y - 6);
                this.octx.lineTo(bounds.x, bounds.y + 6);
                this.octx.stroke();
                ...
                const tx = bounds.x;
                const ty = Math.max(12, bounds.y - 6);
                this.octx.fillRect(tx - 2, ty - 12, textW + 6, 14);
                this.octx.fillStyle = 'white';
                this.octx.fillText(label, tx + 1, ty - 1);
            }
    ...
    function getRotatedRectBounds(layer) {
    ...
    function drawMaskOnOverlay(octx, mask, layer) {
        const angle = Number(layer.angle) || 0;
        const x = Number(layer.x) || 0;
        const y = Number(layer.y) || 0;
        const width = Number(layer.width) || 0;
        ...
        octx.save();
        octx.fillStyle = 'rgba(0,200,0,0.18)';
        if (angle !== 0) {
            octx.translate(x + width / 2, y + height / 2);
            octx.rotate(Math.PI * angle / 180);
    ...
    function getAlphaMask(img, sx, sy, sw, sh, dstW, dstH) {
        try {
            ...
            const id = ctx.getImageData(0, 0, dstW, dstH).data;
            const rows = new Array(dstH);
