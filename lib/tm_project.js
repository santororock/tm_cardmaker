(function(global) {
    ...
    function create() {
        ...
        function moveTo(id, newIndex) {
            const oldIndex = layerOrder.indexOf(id);
            if (oldIndex === -1) return false;
            const boundedIndex = Math.max(0, Math.min(layerOrder.length - 1, newIndex));
            if (oldIndex === boundedIndex) return false;
            layerOrder.splice(oldIndex, 1);
            layerOrder.splice(boundedIndex, 0, id);
            return true;
        }
...
(function(global) {
    ...
    function normalize(layer) {
        ...
        // Aspect-ratio locks are layer preferences. Older projects did not
        // store them, so retain the historical checked-by-default behavior.
        if (layer.lockAspectRatio === undefined) layer.lockAspectRatio = true;
        if (layer.lockSourceAspectRatio === undefined) layer.lockSourceAspectRatio = true;
        return layer;
    }
...
(function(global) {
    ...
    function makeSaveableLayers(layers, dependencies) {
        ...
        for (const layer of layers) {
            ...
            if (copy.parentGroupKey) delete copy.groupId;
            if (copy.type === "block") {
