...
function rebuildBlockMenu() {
    ...
    applyBlockSearchFilter();
}

var blockSearchQuery = "";
var blockSearchOpenCategories = null;

function normalizeBlockSearchText(value) {
    return String(value || "").toLowerCase().replace(/[_/\\-]+/g, " ").replace(/\s+/g, " ").trim();
}

function getBlockSearchText(asset) {
    const setNames = normalizeAssetSetValues(asset.sets).map(function(setId) {
        const canonicalId = resolveSetId(setId);
        const set = assetSets.find(function(candidate) { return candidate.id === canonicalId; });
        return set ? set.name : setId;
    });
    return normalizeBlockSearchText([
        asset.text,
        asset.src,
        formatCategoryLabel(asset.putUnder),
        setNames.join(" ")
    ].join(" "));
}

function applyBlockSearchFilter() {
    const container = document.getElementById("blocksMenu");
    const summary = document.getElementById("blockSearchSummary");
    if (!container) return;
    const tokens = normalizeBlockSearchText(blockSearchQuery).split(" ").filter(Boolean);
    let matchCount = 0;

    container.querySelectorAll(".block-category-list").forEach(function(list) {
        let categoryMatches = 0;
        list.querySelectorAll(".thumbnail-menu-item").forEach(function(item) {
            const matches = tokens.length === 0 || tokens.every(function(token) {
                return (item.dataset.searchText || "").indexOf(token) !== -1;
            });
            item.classList.toggle("block-search-hidden", !matches);
            if (matches) {
                categoryMatches++;
                matchCount++;
            }
        });
        const header = Array.from(container.querySelectorAll(".block-category-header")).find(function(candidate) {
            return candidate.dataset.categoryTarget === list.id;
        });
        if (header) header.classList.toggle("block-search-hidden", tokens.length > 0 && categoryMatches === 0);
        if (tokens.length > 0) {
            list.classList.toggle("block-search-hidden", categoryMatches === 0);
            if (categoryMatches > 0) {
                list.classList.remove("w3-hide");
                list.classList.add("w3-show");
                if (list.id.startsWith("blocks/")) loadCategoryThumbnails(list.id);
            }
        } else {
            list.classList.remove("block-search-hidden");
        }
    });
    if (summary) summary.textContent = tokens.length ? (matchCount ? matchCount + (matchCount === 1 ? " result" : " results") : "No matching blocks") : "";
}

function filterBlockMenu(value) {
    const wasSearching = Boolean(normalizeBlockSearchText(blockSearchQuery));
    blockSearchQuery = value || "";
    const isSearching = Boolean(normalizeBlockSearchText(blockSearchQuery));
    const container = document.getElementById("blocksMenu");
    if (container && isSearching && !wasSearching) {
        blockSearchOpenCategories = new Set();
        container.querySelectorAll(".block-category-list.w3-show").forEach(function(list) { blockSearchOpenCategories.add(list.id); });
    }
    applyBlockSearchFilter();
    if (container && !isSearching && wasSearching) {
        container.querySelectorAll(".block-category-list").forEach(function(list) {
            const shouldOpen = blockSearchOpenCategories && blockSearchOpenCategories.has(list.id);
            list.classList.toggle("w3-show", shouldOpen);
            list.classList.toggle("w3-hide", !shouldOpen);
        });
        blockSearchOpenCategories = null;
    }
}
...
function buildBlockMenu() {
    ...
    finalOrder.forEach((categoryId) => {
        ...
        header.classList.add("block-category-header");
        header.dataset.categoryTarget = categoryId;
        header.innerHTML = `${label} <i class="fa fa-caret-down"></i>`;

        const list = document.createElement("div");
        list.id = categoryId;
        list.className = "w3-bar-block w3-hide w3-padding-large w3-medium";
        list.classList.add("block-category-list");
