    ...
    function renderSingleLayerToContext(ctx, layer) {
        switch (layer.type) {
            ...
            case "block":
                if (layer.obg) {
                    const brdr = 3;
                    const otherBgName = blockList[layer.iNum] && blockList[layer.iNum].otherbg;
                    const otherBg = otherBgList[otherBgName];
                    if (otherBg && otherBg.complete) {
                        ctx.drawImage(otherBg, layer.x - brdr, layer.y - brdr, layer.width + 2 * brdr, layer.height + 2 * brdr);
                    }
                }
                if (blockList[layer.iNum] && blockList[layer.iNum].obj && blockList[layer.iNum].obj.complete) {
