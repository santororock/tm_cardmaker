...
(function() {
    ...
    function getOrderedProjectLayersForExport() {
        ...
        for (let i = 0; i < layerDivs.length; i++) {
            ...
            if (aLayers[id] && aLayers[id].type !== "group" && isLayerEffectivelyVisible(id)) {
    ...
    function buildPsd(exportDoc) {
        ...
        for (let i = 0; i < psdLayers.length; i++) {
            ...
            // Layer pixels retain their source alpha; opacity stays editable in Photoshop.
            record.push(Math.round(layer.opacity * 255));
            record.push(0); // Clipping: base layer.
            record.push(layer.visible ? 0 : 2); // Layer flags.
