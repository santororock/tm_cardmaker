...
function openUserImageCacheDb() {
    if (!supportsUserImageCache()) return Promise.resolve(null);
    if (userImageCacheDbPromise) return userImageCacheDbPromise;

    userImageCacheDbPromise = new Promise(function(resolve, reject) {
        try {
            const request = indexedDB.open(USER_IMAGE_DB_NAME, USER_IMAGE_DB_VERSION);
            request.onupgradeneeded = function(event) {
                const db = event.target.result;
                if (!db.objectStoreNames.contains(USER_IMAGE_STORE)) {
                    const store = db.createObjectStore(USER_IMAGE_STORE, { keyPath: "id" });
    ...
    return userImageCacheDbPromise;
}

function createUserImageCacheId() {
...
function putCachedUserImage(cacheId, filename, blob) {
    if (!cacheId || !blob || !supportsUserImageCache()) return Promise.resolve(false);
    return openUserImageCacheDb().then(function(db) {
        if (!db) return false;
        return new Promise(function(resolve, reject) {
            const tx = db.transaction([USER_IMAGE_STORE], "readwrite");
            const store = tx.objectStore(USER_IMAGE_STORE);
            const record = {
                id: cacheId,
                filename: filename || "",
                mimeType: blob.type || "",
            ...
            tx.onerror = function(event) { reject(event.target.error || new Error("Failed to cache image")); };
        });
    });
}

function getAllCachedUserImageRecords() {
    if (!supportsUserImageCache()) return Promise.resolve([]);
    return openUserImageCacheDb().then(function(db) {
        if (!db) return [];
        return new Promise(function(resolve, reject) {
            try {
                const tx = db.transaction([USER_IMAGE_STORE], "readonly");
                const store = tx.objectStore(USER_IMAGE_STORE);
                ...
                req.onerror = function(event) {
                    reject(event.target.error || new Error("Failed to read cached images"));
                };
            } catch (error) {
                reject(error);
            }
...
function clearAllCachedUserImages() {
    if (!supportsUserImageCache()) return Promise.resolve(false);
    return openUserImageCacheDb().then(function(db) {
        if (!db) return false;
        return new Promise(function(resolve, reject) {
            const tx = db.transaction([USER_IMAGE_STORE], "readwrite");
            ...
            req.onerror = function(event) { reject(event.target.error || new Error("Failed to clear cached images")); };
        });
    });
}
...
var megaTemplates = {
    ...
    cosmic_horror_normal: {
        layers: [
            ...
            { type: "text", data: "Card description\nMultiple lines\nand they can be much, much, much longer\n'V space' controls the spacing between lines", x: 110, y: 770, width: 826, height: 26, color: "#000000", font: TM_SERIF_BODY_FONT, style: "normal", weight: "normal", lineSpace: 4, justify: "left", params: "allimages color alltext allangle allpreset" },
            ...
            { type: "text", data: "Flavor text!", x: 413, y: 1005, width: 826, height: 26, color: "#000000", font: TM_SERIF_BODY_FONT, style: "italic", weight: "bold", lineSpace: 4, justify: "center", params: "allimages color alltext allangle allpreset" }
        ]
    },
    prelude: {
        layers: [
...
function addLayer(title, layer) {
    ...
    childAdd.appendChild(createLayerVisibilityButton(layer));
    ...
    if (ddcount) childAdd.appendChild(createLayerDragHandle());
...
function updateLayerVisibilityUI(layerNode, layer) {
    if (!layerNode || !layer) return;
    const visible = isLayerVisible(layer);
    layerNode.classList.toggle("layer-hidden", !visible);
    const button = layerNode.querySelector(".layer-visibility-button");
    if (!button) return;
    button.innerHTML = layerVisibilityIcon(visible);
    button.title = visible ? "Hide layer" : "Show layer";
    button.setAttribute("aria-label", button.title);
    button.setAttribute("aria-pressed", visible ? "true" : "false");
}

function createLayerVisibilityButton(layer) {
    const button = document.createElement("button");
    button.type = "button";
    button.className = "layer-visibility-button";
    button.onclick = function(e) {
        e.stopPropagation();
        const layerNode = this.closest(".divRec");
        if (!layerNode || !aLayers[layerNode.id]) return;
        const targetLayer = aLayers[layerNode.id];
        ...
        updateLayerVisibilityUI(layerNode, targetLayer);
        drawProject();
        captureState(label);
    };
    button.innerHTML = layerVisibilityIcon(isLayerVisible(layer));
    return button;
}
...
function createLayerDragHandle() {
    let toAdd = document.createElement("button");
    toAdd.type = "button";
    toAdd.className = "layer-drag-handle";
    toAdd.title = "Drag to reorder layer";
    toAdd.setAttribute("aria-label", "Drag to reorder layer");
    toAdd.innerHTML = "<span></span><span></span><span></span><span></span><span></span><span></span>";
    toAdd.onclick = function(e) { e.stopPropagation(); };
    return toAdd;
}
...
function refreshParamsForLayer(layer, nodeId) {
    if (!layer) return;
    let thisLayer = layer;
    if (thisLayer.visible === undefined) thisLayer.visible = true;
    if (thisLayer.alpha === undefined) thisLayer.alpha = 100;
...
function rebuildLayerListUI() {
    ...
    for (let layerId of layerOrder) {
        if (aLayers[layerId]) {
            ...
            childDiv.appendChild(createLayerVisibilityButton(layer));
            ...
            if (layer.type !== "base") {
                childDiv.appendChild(createLayerDragHandle());
            }

            li.appendChild(childDiv);
            layerList.appendChild(li);
            updateLayerVisibilityUI(li, layer);
        }
    }
...
function ensureBlockColorDefaults(layer) {
    ...
    for (let key in blockColorDefaultValues) {
        if (layer[key] === undefined || layer[key] === "") {
            layer[key] = blockColorDefaultValues[key];
        }
    }
}
...
function getBlockColorFilter(layer) {
    const hue = numberOrDefault(layer.hue, blockColorDefaultValues.hue);
    const saturation = numberOrDefault(layer.saturation, blockColorDefaultValues.saturation);
    const lightness = numberOrDefault(layer.lightness, blockColorDefaultValues.lightness);
...
function getLayerGamma(layer) {
    return Math.max(1, numberOrDefault(layer.gamma, blockColorDefaultValues.gamma));
}
...
function drawBlockLayer(ctx, layer) {
    ...
    function drawBlockContents(targetCtx) {
        if (layer.obg && thisBlock.otherbg) {
            ...
            if (!otherBg) {
                const otherBgIndex = findOtherBgBlockIndex(thisBlock.otherbg);
                if (otherBgIndex !== -1) {
                    fetchBlock(otherBgIndex);
                } else {
                    console.warn("otherbg image not found for", thisBlock.otherbg, thisBlock);
                }
            } else if (otherBg.complete) {
        ...
        } else if (thisBlock.obj.complete) {
...
function drawUserImageLayer(ctx, layer) {
    if (layer.iNum == -1 || !userImageList[layer.iNum]) return;
    ensureBlockColorDefaults(layer);
...
function drawProject() {
    ...
    for (let i = 0; i < layerDivs.length; i++) {
        ...
        // The base always defines the document size, even when its paint is hidden.
        if (layer.type === "base") {
            c.height = layer.height;
            c.width = layer.width;
            if (isLayerVisible(layer) && getLayerOpacity(layer) > 0) {
                ctx.save();
        ...
        // A first block can also serve as the legacy canvas-size layer.
        ...
        if (!isLayerVisible(layer) || getLayerOpacity(layer) <= 0) continue;
        ctx.save();
        ctx.globalAlpha = getLayerOpacity(layer);
        switch (layer.type) {
            case "block":
                // layer = {type:"block", obj:{}, x:0, y:0, width:0, height:0, params:"allimages"};
                drawBlockLayer(ctx, layer);
                break;
            case "text":
