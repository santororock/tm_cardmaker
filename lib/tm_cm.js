...
function applyValueToLayer(th, captureUndo) {
    ...
    if (!aLayers[layerName] || aLayers[layerName].locked) {
        ...
        if ((fieldName == "x") || (fieldName == "y")) {
            ...
            if (layer.getElementsByClassName("groupcheck")[0].checked) {
                ...
                for (let x = 0; x < thisgroup.length; x++) {
                    ...
                    if (aLayers[groupLayerId].locked) continue;
...
function drag(event) {
    ...
    if (layerToDrag != null && !layerToDrag.locked) {
        layerToDrag.x = x;
        layerToDrag.y = y;
        drawProject();
...
function moveLayerWithKey(event) {
    if (keyFocusLayer != null && !keyFocusLayer.locked) {
