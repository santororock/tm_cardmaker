...
function ensureBlockColorDefaults(layer) {
    if (!layer || layer.type != "block") return;
...
function drawBlockLayer(ctx, layer) {
    ...
    if (layer.obg && thisBlock.otherbg) {
        ...
        const padX = Number.isFinite(layerPadX) ? layerPadX : (Number.isFinite(spritePadX) ? spritePadX : 3);
        const padY = Number.isFinite(layerPadY) ? layerPadY : (Number.isFinite(spritePadY) ? spritePadY : 3);

        if (!otherBgList[thisBlock.otherbg]) {
            for (let j = 0; j < blockList.length; j++) {
                if (blockList[j].text == thisBlock.otherbg) {
                    fetchBlock(j);
                    break;
                }
            }
        } else if (otherBgList[thisBlock.otherbg].complete) {
            ctx.drawImage(otherBgList[thisBlock.otherbg], -padX, -padY, width + 2 * padX, height + 2 * padY);
        }
    }
    ...
    if (!thisBlock.obj) {
        ...
        ctx.drawImage(thisBlock.obj, 0, 0, width, height);
    }
    ctx.restore();
}
...
function addBlock(th) {
    ...
    let layer = { type: "block", name: "", iNum: 0, x: 0, y: 0, width: 0, height: 0, angle: 0, hue: 0, saturation: 100, lightness: 100, params: "allimages allangle blockcolor" };
    let myIndex = 0;
    if ((typeof th == "number")) {
        myIndex = th;
