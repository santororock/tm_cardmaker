...
var dragOffsetX;
var dragOffsetY;
var layerDragStartMouse = null;
var layerDragStartPositions = null;
var keyFocusLayer;
...
function addBlockMenuItem(num) {
    ...
    if (!isAssetVisible(blockList[num])) {
        ...
        if (!document.getElementById("image" + num)) {
            ...
            toAdd.dataset.searchText = getBlockSearchText(blockList[num]);
            ...
            if (blockList[num].putUnder == "blocks/templates") {
                ...
                // Reuse the block row's thumbnail and label layout.
                ...
                // The File submenu opens independently of the block categories.
                loadThumbnail(toAdd.querySelector(".thumbnail-container"),
                    blockList[num].src, blockList[num].putUnder, num);
            }
        }
    }
}
...
function drawProject() {
    ...
    const renderLayers = orderedLayerIds.filter(function(layerId) {
        return aLayers[layerId] && !isGroupLayer(aLayers[layerId]) && isLayerEffectivelyVisible(layerId);
    }).map(function(layerId) { return aLayers[layerId]; });
    ...
    if (window.debugOverlay && typeof window.debugOverlay.drawOverlay === 'function') {
        try {
            window.debugOverlay.drawOverlay(canvas, renderLayers, aLayers);
        } catch (err) {
...
function setAutosaveStatus(state, message, detail) {
    ...
    try {
        ...
        if (typeof(Storage) !== "undefined") {
            ...
            autosaveStatusTimer = setTimeout(function() {
                ...
                setAutosaveStatus("saved", "Saved locally", "Autosaved locally at " + savedAt.toLocaleTimeString());
            }, 180);
        } else {
            setAutosaveStatus("unavailable", "Autosave unavailable", "This browser does not provide local storage.");
        }

    } catch (error) {
        console.error("Autosave failed", error);
        setAutosaveStatus("error", "Autosave failed", error && error.message ? error.message : "The project could not be saved locally.");
    }
}
...
function getLayerNodeForParamInput(th) {
    return th && typeof th.closest === "function" ? th.closest(".divRec") : null;
}
...
function applyValueToLayer(th, captureUndo) {
    ...
    let fieldName = th.id === "lar"
        ? "lockAspectRatio"
        : th.id === "slar"
            ? "lockSourceAspectRatio"
            : th.id.slice(5);
    if (!aLayers[layerName] || isLayerEffectivelyLocked(layerName)) {
        refreshParamsForLayer(aLayers[layerName], layerName);
        return;
    }
    ...
    if ((th.type == "number") || (th.type == "range")) {
        ...
        if ((fieldName == "x") || (fieldName == "y")) {
            ...
            getSelectedLayerIds(true).forEach(function(selectedId) {
                if (selectedId === layerName || !aLayers[selectedId] || isLayerEffectivelyLocked(selectedId)) return;
                aLayers[selectedId][fieldName] += diff;
            });
        }
...
function setPresets(th) {
    ...
    let layerDom = getLayerNodeForParamInput(th);
    if (!layerDom) return;
...
function onBlockLoad() {
    ...
    for (let l in aLayers) {
        if ((aLayers[l].type == "block") && (aLayers[l].iNum == this.dataindex)) {
            ...
            if (pendingAutoSizedBlockLayers.has(aLayers[l])) {
                const centerX = Number(aLayers[l].x) + Number(aLayers[l].width) / 2;
                const centerY = Number(aLayers[l].y) + Number(aLayers[l].height) / 2;
                aLayers[l].width = scaledW;
                aLayers[l].height = scaledH;
                aLayers[l].x = Math.round(centerX - scaledW / 2);
                aLayers[l].y = Math.round(centerY - scaledH / 2);
                pendingAutoSizedBlockLayers.delete(aLayers[l]);
            } else if ((!aLayers[l].width && !aLayers[l].height) || (aLayers[l].height === 100 && !aLayers[l].width)) {
                aLayers[l].width = scaledW;
                aLayers[l].height = scaledH;
...
function loadFrom(saved, autoload) {
    ...
    try {
        ...
        const loadedGroupIdsByKey = {};
        for (let layer of savedLayers) {
            if (layer.parentGroupKey && loadedGroupIdsByKey[layer.parentGroupKey]) {
                layer.groupId = loadedGroupIdsByKey[layer.parentGroupKey];
            }
            let ignore = ["type", "params"];
            let newLayer = {};
            switch (layer.type) {
                case "group":
                    addLayer(layer.name || "Group", layer);
                    if (layer.groupKey) loadedGroupIdsByKey[layer.groupKey] = "dragdropdiv" + (ddcount - 1);
                    break;
                case "block":
                case "text":
                case "production":
                ...
                case "line":
                    ...
                    if (layer.type == "block") pendingAutoSizedBlockLayers.delete(newLayer);
                    break;
                    // case "group":
                ...
                case "userFile":
                    if (autoload) {
                        ...
                        if (cachedIndex !== -1) {
                            ...
                            addLayer("Artwork: " + layer.filename, layer);
                        } else {
                            unreloadable = true;
                        }
                        break;
            ...
            if (loadedLayerNode && aLayers[loadedLayerNode.id]) {
                ...
                updateLayerLockedUI(loadedLayerNode, aLayers[loadedLayerNode.id]);
                const parentGroupId = aLayers[loadedLayerNode.id].groupId;
                if (parentGroupId && aLayers[parentGroupId]) {
                    loadedLayerNode.classList.add("group-child");
                    loadedLayerNode.dataset.groupId = parentGroupId;
                    updateGroupUI(document.getElementById(parentGroupId), aLayers[parentGroupId]);
                }
            }
            if (layer.name) {
...
const pendingAutoSizedBlockLayers = new WeakSet();

function getNewBlockDimensions(block) {
    const image = block && block.obj && block.obj.complete ? block.obj : null;
    const sourceWidth = Number(image && (image.naturalWidth || image.width)) || Number(block && block.width);
    const sourceHeight = Number(image && (image.naturalHeight || image.height)) || Number(block && block.height);
    ...
    return { width: 100, height: 100, pending: true };
}

function applyNewBlockDefaults(layer, block, defaultsKey, canvas) {
    const initialSize = getNewBlockDimensions(block);
    const presets = blockDefaults[defaultsKey];
    const preset = Array.isArray(presets) && presets.length ? presets[0] : null;
    layer.width = initialSize.width;
    layer.height = initialSize.height;

    if (preset) {
        Object.keys(preset).forEach(function(property) {
            if (property !== "label") layer[property] = preset[property];
        });

        const sourceWidth = Number(block && block.width) || Number(block && block.obj && block.obj.width);
        const sourceHeight = Number(block && block.height) || Number(block && block.obj && block.obj.height);
        const hasPresetWidth = Number.isFinite(Number(preset.width));
        const hasPresetHeight = Number.isFinite(Number(preset.height));
        if (sourceWidth > 0 && sourceHeight > 0) {
            if (hasPresetHeight && !hasPresetWidth) layer.width = Math.max(1, Math.round(Number(layer.height) * sourceWidth / sourceHeight));
            if (hasPresetWidth && !hasPresetHeight) layer.height = Math.max(1, Math.round(Number(layer.width) * sourceHeight / sourceWidth));
        }
    }

    if (!preset || !Number.isFinite(Number(preset.x))) layer.x = Math.round((canvas.width - Number(layer.width)) / 2);
    if (!preset || !Number.isFinite(Number(preset.y))) layer.y = Math.round((canvas.height - Number(layer.height)) / 2);

    const presetHasCompleteSize = preset && Number.isFinite(Number(preset.width)) && Number.isFinite(Number(preset.height));
    return { pending: initialSize.pending && !presetHasCompleteSize };
}
...
function addBlock(th) {
    ...
    const blockCategoryNames = {
        tags: "Tag", requisites: "Requirement", requirements: "Requirement",
        productionboxes: "Production", resources: "Resource", tiles: "Tile",
        parties: "Party", colonies: "Colony", VPs: "VP", templates: "Template"
    };
    const categoryKey = String(thisBlock.putUnder || "").replace(/^blocks\//, "");
    const categoryName = blockCategoryNames[categoryKey] || "Block";
    const displayName = String(thisBlock.text || thisBlock.src || "Block").replace(/_/g, " ").replace(/\s+/g, " ").trim();
    const canvas = document.getElementById("cmcanvas");
    const initialPlacement = applyNewBlockDefaults(layer, thisBlock, defaultsKey, canvas);
    let newLayer = addLayer(categoryName + ": " + displayName, layer);
    if (initialPlacement.pending) pendingAutoSizedBlockLayers.add(newLayer);
    if (!thisBlock.obj || !thisBlock.obj.complete) {
        fetchBlock(layer.iNum);
    }
    drawProject();

    const userInitiated = typeof Event !== "undefined" && th instanceof Event;
    if (userInitiated) {
        th.preventDefault();
        const newLayerId = projectState.getOrder().find(function(id) { return aLayers[id] === newLayer; });
        if (newLayerId) selectLayerById(newLayerId);
    }
...
function addTextBox(th) {
    ...
    layer.width = c.width;
    const textName = layer.data === "Replace this text!"
        ? "Text"
        : "Text: " + String(layer.data).replace(/\s+/g, " ").trim().slice(0, 28);
    let newLayer = addLayer(textName, layer);
    drawProject();
    return newLayer;
}
...
function userImageLoaded() {
    ...
    if (projectLoad) {
        ...
        let newLayer = addLayer("Artwork: " + layer.filename, layer);
        drawProject();
    }
}
...
function addEmbed() {
    ...
    let newLayer = addLayer("Artwork: Web image", layer);
...
function dragStart(event) {
    ...
    if (beginCanvasControlDrag(mouse)) {
        layerToDrag = null;
        removeKeyInputFocus();
    ...
    for (let i = orderedLayerIds.length - 1; i >= 0; i--) {
        ...
        // Locked and hidden layers remain selectable from the layer panel, but
        // canvas clicks pass through them to the next interactive layer below.
        if (!layer || isGroupLayer(layer) || isLayerEffectivelyLocked(layerId) || !isLayerEffectivelyVisible(layerId)) continue;
        if (clickIsWithinLayer(layer, mouse.x, mouse.y)) {
            // Select the corresponding layer in the properties panel.
            // Canvas clicks target the containing group; individual children
            // remain directly selectable from the expanded layer panel.
            const selectionId = layer.groupId && aLayers[layer.groupId] ? layer.groupId : layerId;
            // `selectLayer` expects to be called with the `.inside` element as `this`.
            let inside = document.getElementById(selectionId).getElementsByClassName('inside')[0];
            if (inside) {
                selectLayer.call(inside, event, true);
            } else {
                selectLayer();
            }
            layerToDrag = layer;
            ...
            layerDragStartMouse = { x: mouse.x, y: mouse.y };
            layerDragStartPositions = {};
            getSelectedLayerIds(true).forEach(function(selectedId) {
                const selectedLayer = aLayers[selectedId];
                if (!selectedLayer || isGroupLayer(selectedLayer) || isLayerEffectivelyLocked(selectedId) || !isLayerEffectivelyVisible(selectedId)) return;
                layerDragStartPositions[selectedId] = { x: Number(selectedLayer.x) || 0, y: Number(selectedLayer.y) || 0 };
            });
            return;
        }
    }
...
function dragEnd(event) {
    if (endCanvasControlDrag()) return;
    if (layerToDrag) {
        console.log("[UNDO] dragEnd triggered - capturing drag state");
        captureState("Drag layer");
    }
    layerToDrag = null;
    layerDragStartMouse = null;
    layerDragStartPositions = null;
}

function drag(event) {
    var mouse = getMousePos(event)
    if (updateCanvasControlDrag(mouse)) {
        event.preventDefault();
        return;
    }
    if (layerToDrag != null && layerDragStartMouse && layerDragStartPositions) {
        const deltaX = mouse.x - layerDragStartMouse.x;
        const deltaY = mouse.y - layerDragStartMouse.y;
        Object.keys(layerDragStartPositions).forEach(function(layerId) {
            const target = aLayers[layerId];
            if (!target || isLayerEffectivelyLocked(layerId) || !isLayerEffectivelyVisible(layerId)) return;
            target.x = layerDragStartPositions[layerId].x + deltaX;
            target.y = layerDragStartPositions[layerId].y + deltaY;
        });
        drawProject();
...
function focusKeyInput(layer) {
    if (!layer || layer.locked || !isLayerVisible(layer)) return;
    keyFocusLayer = layer;
...
function moveLayerWithKey(event) {
    // Leave navigation and editing keys to controls, and pause canvas shortcuts
    // while the pointer or keyboard focus is in the properties panel.
    const panel = document.getElementById("rightPanel");
    const isEditing = (element) => element && (
        element.isContentEditable ||
        (element.closest && element.closest("input, textarea, select"))
    );
    if (event.defaultPrevented || isEditing(event.target) || isEditing(document.activeElement) ||
        (panel && (panel.matches(":hover") || panel.contains(event.target) || panel.contains(document.activeElement)))) {
        return;
    }
    if (keyFocusLayer != null && !keyFocusLayer.locked && isLayerVisible(keyFocusLayer)) {
        ...
        if (delta.x || delta.y || delta.w || delta.h) {
            const selectedIds = getSelectedLayerIds(true);
            if (!event.altKey && selectedIds.length > 1) {
                selectedIds.forEach(function(layerId) {
                    const selectedLayer = aLayers[layerId];
                    if (!selectedLayer || isLayerEffectivelyLocked(layerId) || !isLayerEffectivelyVisible(layerId)) return;
                    selectedLayer.x += delta.x;
                    selectedLayer.y += delta.y;
                });
                drawProject();
                captureState("Nudge " + selectedIds.length + " layers");
                event.preventDefault();
                return;
            }
